...
| Post-merge `main` | **SUCCESS** — run `33970304195` at `f43d8d1`, every job green, none skipped-with-failure. A green PR is not a green `main` on shared mutable state, so this was watched rather than assumed; an earlier revision of this report asserted it green *before* it had finished, and was corrected on the relay rather than left standing. That is the row's whole purpose, and it caught its own author. |
