...
| **P2** (P2a → P2b-3) | **closed** 2026-09-11 | Web DB schema v2 under RLS (W153, W159); the first on-demand route `/api/health` on Hobby (W158). Carried: F50/F51, F64, F68, Dependabot #83/#84 (W114; #84 a major) |
...
- **First:** F51/F50 with the new token (GATE 1); then F67, F70.
...
| F51 | med | SKU backfill + `required()`: one GATE 1 script; needs `SANITY_WRITE_TOKEN` | P3's first chunk: run once as approved, re-fetched live, `required()` ships |
...
| F67 | low | The function also serves Astro's `/_image`: visitors can make it resize our images (compute, no data; no page uses it) | P3 rules on disabling it in `astro.config.mjs` |
...
| F70 | low | Advisors (#90): `function_search_path_mutable` ×2, `authenticated_security_definer_function_executable` ×8; `security_definer_view` is intended (W159) | P3's first migration pins `search_path`, rules on the 8 |
